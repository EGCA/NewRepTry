...
function initializeTeamFilters() {
  ...
  const teamsData = [
    ...
    },
    {
      ...
      founded: 2010,
      achievements: ['Greek Cup Champions 2024'],
    ...
    },
    {
      ...
      coach: 'Ioannis Papas',
      founded: 2011,
      achievements: ['Regional Cup 2022'],
      description: 'A.E.T. Pyrsos brings local passion and skilled players.'
    },
    {
      id: 'nais',
      name: 'Nais',
      ...
      founded: 2011,
      achievements: ['Serbian National Champions 2024'],
    ...
    },
    {
      ...
      coach: 'Jan de Vries',
      founded: 2009,
      achievements: ['Dutch Cup Finalist 2023'],
      description: 'USV Hercules is a disciplined and tactical squad from the Netherlands.'
    },
    {
      id: 'fsbu_gothenburg',
      ...
      coach: 'Lars Svensson',
      founded: 2013,
      achievements: ['Nordic Semi-Final 2022'],
      description: 'FSBU Gothenburg is known for quick transitions and solid defense.'
    }
  ];

  function renderTeams(list) {
    if (!teamsContainer) return;
    teamsContainer.innerHTML = list.map(team => `
      <div class="team-card" data-team="${team.id}" data-region="${team.region}">
        <div class="team-logo">
          <img src="${team.logo}" alt="${team.name} logo" class="team-card-img">
        </div>
        <h3 class="team-name">${team.name}</h3>
        <p class="team-nation">${team.country}</p>
        <div class="team-info">
  ...
  filterButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const list = filter === 'all' ? teamsData : teamsData.filter(t => t.region === filter);
      renderTeams(list);
    });
  });

  // Expose for modal use
  window.__egcaTeams = teamsData;
}
...
function openTeamModal(teamId) {
  ...
  const teams = window.__egcaTeams || [];
  const team = teams.find(t => t.id === teamId) || teams[0];

  if (modalTeamName) modalTeamName.textContent = team.name;
  if (modalTeamFlag) modalTeamFlag.textContent = team.country;

  if (modalPlayers) {
    modalPlayers.innerHTML = `
      <div class="player-card">
        <div class="player-name">Coach: ${team.coach || 'TBA'}</div>
        <div class="player-position">Founded: ${team.founded || '—'}</div>
      </div>
      <div class="player-card" style="grid-column: 1 / -1;">
        <div class="player-name">About</div>
        <div class="player-position">${team.description || ''}</div>
      </div>
    `;
  }

  if (modalStaff) {
    const ach = (team.achievements || [])
      .map(a => `<div class="staff-item"><div class="staff-name">• ${a}</div><div class="staff-role">Achievement</div></div>`)
      .join('');
    modalStaff.innerHTML = ach || '<p>No recent achievements listed.</p>';
  }
